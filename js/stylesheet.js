...
chrome.extension.sendRequest({elements: 'o'}, function(response) {
	...
	css += "#main, #search, #logo-container { margin-top: 5px !important; }"
	...
	// GENERAL
		...
		if (response.o.borders) {
			...
			css += "#viewer-container, #entries.list .read .collapsed, #entries.list .entry .entry-actions { background: " + lighten(lighten(lighten(response.o.borderCLR))) + " !important; background-color: " + lighten(lighten(lighten(response.o.borderCLR))) + " !important; }\n";
	...
	// GOOGLE BAR
		...
		if (response.o.gbarO) {
			css += "#gbz, #gbg, #gbx3, #gbx4 ,#guser, #gbar { opacity: 0; -webkit-transition-delay: .25s; }\n";
			css += "#gb:hover #gbz, #gb:hover #gbg, #gb:hover #gbx3, #gb:hover #gb:hover #gbx4, #gbar:hover, #guser:hover { opacity: 1 !important; }\n"
		}
		if (response.o.gbarB)
